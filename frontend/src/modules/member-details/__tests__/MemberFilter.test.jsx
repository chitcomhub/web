...
it("Renders member data simple case", async () => {
	...
	let userMainInfoBlock = document.querySelector(
		".user-card .block-user div"
	);
	expect(userMainInfoBlock.querySelector("h5").textContent).toBe(
		fakeMember.first_name + " " + fakeMember.last_name
	);
	expect(userMainInfoBlock.querySelector("p").textContent).toBe(
		fakeMember.short_bio
	);

	// check social urls present and correct
	let socialLinks = document
		.querySelector(".user-card .block-socials")
		.querySelectorAll("a");
	...
	expect(urls.sort()).toEqual(
		[
			"https://t.me/" + fakeMember.telegram,
			"https://github.com/" + fakeMember.github,
		].sort()
	);
	// remove the mock
